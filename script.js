// BASMALLAH RENTALS — Complete Application v4.0
// Single clean script, no patches
// ════════════════════════════════════════════════
...
var bookings  = [];
var equipment = [
  { id:1, name:'White Monobloc', cat:'Chairs', price:100, stock:50, unit:'pcs', icon:'🪑', desc:'Budget-friendly', badge:'new' },
  { id:2, name:'Executive Cushion', cat:'Chairs', price:250, stock:30, unit:'pcs', icon:'🪑', desc:'Premium comfort', badge:'' }
];
var enquiries = [];
var cart = {};
var chatHist = [];
    ...
    return '<div class="cat-sec">'
      ...
      + cats[cat].map(function(e) {
        ...
        return '<div class="eq-card" onclick="openModal(' + e.id + ')">'
          + (src ? '<img src="' + src + '" alt="' + e.name + '" style="width:100%;height:150px;object-fit:cover;border-radius:8px;">' : '<div style="width:100%;height:150px;background:var(--gp);display:flex;align-items:center;justify-content:center;font-size:2rem;border-radius:8px;">' + e.icon + '</div>')
...
function renderFeatured() {
  ...
  con.innerHTML = equipment.slice(0, 4).map(function(e) {
    ...
    return '<div class="feat-card" onclick="openModal(' + e.id + ')">'
      + (src ? '<img src="' + src + '" alt="' + e.name + '">' : '<div style="width:100%;height:200px;background:var(--gp);display:flex;align-items:center;justify-content:center;font-size:3rem;">' + e.icon + '</div>')
      + '<h4>' + e.name + '</h4>'
      + '<p>₦' + e.price + '/' + e.unit + '</p>'
...
function renderGallery() {
  ...
  con.innerHTML = equipment.slice(0, 8).map(function(e) {
    ...
    return '<div class="gallery-item"' + span + '>'
      + (src ? '<img src="' + src + '" alt="' + e.name + '">' : '<div style="width:100%;height:100%;background:var(--gp);display:flex;align-items:center;justify-content:center;font-size:2rem;">' + e.icon + '</div>')
      + '</div>';
  }).join('');
}
...
function openModal(id) {
  ...
  if (m) {
    m.querySelector('.modal-img').innerHTML = (function() {
      var src = getImgSrc(currentModalItem);
      return src ? '<img src="' + src + '" style="width:100%;border-radius:8px;">' : '<div style="width:100%;height:200px;background:var(--gp);display:flex;align-items:center;justify-content:center;font-size:3rem;border-radius:8px;">' + currentModalItem.icon + '</div>';
    })();
...
function submitBooking() {
  ...
  bookings.unshift({ id:id, fn:f.fn, ln:f.ln, phone:f.phone, email:f.email,
    evtype:f.evtype, date:f.date, dur:f.dur, venue:f.venue, guests:f.guests,
    notes:f.notes, platform:f.platform, status:'pending' });
  sendEmail({ to_name:f.fn+' '+f.ln, to_email:f.email||ADMIN_EMAIL,
    event_type:f.evtype, event_date:f.date, venue:f.venue,
    items:buildItemList(), phone:f.phone, notes:f.notes||'None' });
  var body = document.getElementById('booking-form-body');
  if (body) body.innerHTML = '<div class="success-wrap">'
    + '<div class="success-icon">🎉</div>'
    + '<h2>Booking Received!</h2>'
    + '<p>Thank you, <strong>' + f.fn + '!</strong> Your booking for <em>' + f.evtype + '</em> on <strong>' + f.date + '</strong> has been submitted.<br><br>'
    + 'We will confirm via <strong>WhatsApp</strong> at <strong>' + f.phone + '</strong> within 2 hours.</p>'
    + '<div style="display:flex;gap:.8rem;justify-content:center;flex-wrap:wrap;">'
    + '<button class="btn btn-wa" onclick="openWA(\'Assalamu alaykum! I booked (Ref #BK' + id + ') for ' + f.evtype + ' on ' + f.date + ' at ' + f.venue + '. Name: ' + f.fn + ' ' + f.ln + '. Please confirm.\')">'
    + '💬 Confirm on WhatsApp</button>'
    + '<button class="btn btn-grn" onclick="goto(\'home\')">'
    + '← Home</button>'
    + '</div></div>';
  cart = {};
  renderOrder();
  updatePriceBox();
  toast('✅ Booking submitted successfully!', 'ok');
}
...
function renderAdmin() {
  ...
  setEl('k-bk',  bookings.length);
  setEl('k-pd',  pending);
  setEl('k-eq',  equipment.length);
  setEl('k-enq', enquiries.length);
  ...
  if (dtb) dtb.innerHTML = bookings.slice(0,6).map(function(b) {
    return '<tr>'
      ...
      + '<td>'
      +   '<button class="act-btn success" onclick="confirmBk(' + b.id + ')">✓ Confirm</button>'
  ...
  if (btb) btb.innerHTML = bookings.map(function(b) {
    return '<tr>'
      ...
      +   '<button class="act-btn" onclick="contactBk(' + b.id + ')">💬</button>'
      +   '<button class="act-btn danger" onclick="cancelBk(' + b.id + ')">✕</button>'
      + '</td></tr>';
  }).join('');
  ...
  if (etb) etb.innerHTML = equipment.map(function(e) {
    ...
    return '<tr>'
      ...
      + '<td style="white-space:nowrap;">'
      +   '<label class="act-btn" style="cursor:pointer;">📷 Photo<input type="file" accept="image/*" style="display:none;" onchange="updEqImg(' + e.id + ',this)"></label>'
      +   '<button class="act-btn danger" onclick="delEq(' + e.id + ')">✕</button>'
      + '</td></tr>';
  }).join('');
  ...
  if (enqtb) enqtb.innerHTML = enquiries.map(function(e) {
    return '<tr>'
      ...
      + '<td>' + e.date + '</td>'
      + '<td>'
      +   '<button class="act-btn" onclick="replyEnq(\'' + (e.email || '') + '\', \'' + e.subj + '\')">Reply</button>'
      +   '<button class="act-btn" onclick="openWA(\'Assalamu alaykum ' + e.fn + '! This is Basmallah Rentals regarding: ' + e.subj + '\')">💬</button>'
      + '</td></tr>';
  }).join('');
}

function setEl(id, val) { var el=document.getElementById(id); if(el) el.textContent=val; }
function confirmBk(id) { var b=bookings.find(function(x){return x.id===id;}); if(b){b.status='confirmed';renderAdmin();toast('✅ Booking #BK'+id+' confirmed','ok');} }
function cancelBk(id)  { var b=bookings.find(function(x){return x.id===id;}); if(b){b.status='cancelled';renderAdmin();toast('Booking cancelled','err');} }
...
function delEq(id) { equipment=equipment.filter(function(e){return e.id!==id;}); renderAdmin(); toast('Equipment removed','ok'); }
function updEq(id, field, val) { var e=equipment.find(function(x){return x.id===id;}); if(e){e[field]=val;toast('✅ '+e.name+' '+field+' updated','ok');} }
function exportCSV() {
  var hdr = ['Ref','First','Last','Phone','Email','Event','Date','Duration','Venue','Guests','Platform','Status','Notes'];
  var rows = bookings.map(function(b) {
    return ['#BK'+b.id,b.fn,b.ln,b.phone,b.email||'',b.evtype,b.date,b.dur||'1 Day',b.venue,b.guests,b.platform,b.status,(b.notes||'').replace(/,/g,';')];
  });
  ...
  a.click();
  toast('✅ Bookings exported as CSV!', 'ok');
}
...
function prevImg(input) {
  ...
  r.onload = function(ev) {
    ...
    var prev = document.getElementById('ae-prev');
    var ph   = document.getElementById('ae-ph');
    var zone = document.getElementById('ae-zone');
...
function clearImg() {
  ...
  var ph   = document.getElementById('ae-ph');
  var zone = document.getElementById('ae-zone');
  var inp  = document.getElementById('ae-file');
...
function addEquip() {
  ...
  var name  = g('ae-name');
  var cat   = g('ae-cat');
  var price = parseInt(g('ae-price')) || 0;
  var stock = parseInt(g('ae-stock')) || 0;
  var unit  = g('ae-unit');
  var icon  = g('ae-icon') || '📦';
  var desc  = g('ae-desc');
  var badge = g('ae-badge');
...
function removeTyping() {
  var t = document.getElementById('typing-dot');
  if (t) t.remove();
}
...
function toggleChat() {
  ...
  if (chatOpen && document.getElementById('chat-msgs').children.length === 0) {
    setTimeout(function() {
      addChatMsg('Assalamu alaykum! 🌙 Welcome to Basmallah Rentals. How can I help with your event today?', 'bot');
    }, 350);
  }
}
...
function sendEmail(params) {
  if (window.emailjs) {
    emailjs.send('YOUR_SERVICE_ID', 'YOUR_TEMPLATE_ID', params, 'YOUR_PUBLIC_KEY').catch(function(){});
  } else {
...
function toast(msg, type) {
  ...
  var icon = type==='ok' ? '✅' : type==='err' ? '❌' : type==='info' ? 'ℹ️' : '💬';
...
document.addEventListener('click', function(ev) {
  if (ev.target && ev.target.id === 'item-modal') closeModal();
});
